...
*   **Auth:** `AUTH_RATE_LIMIT_PER_MINUTE=30/BURST=10`, `GOOGLE_*`, `ALLOWED_EMAIL_DOMAIN`, `JWT_ACCESS_SECRET`, `ACCESS_TOKEN_TTL_MINUTES=60`, `REFRESH_TOKEN_TTL_HOURS=72` (3 hari + sliding), `FRONTEND_URL`, `COOKIE_*`
...
*   `AllowedOrigins` (http/https, untuk `AllowOrigins`) + `AllowedCustomOrigins`
    (custom-scheme, untuk `AllowOriginFunc`) via
