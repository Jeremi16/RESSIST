...
// MoodleCalendarEvent represents a parsed Moodle ICS calendar event.
...
// GoogleDate represents a Google Classroom due date.
...
// GoogleTimeOfDay represents a Google Classroom due time.
...
// Exported version of parseMoodleICS.
func ParseMoodleICS(raw string) []MoodleCalendarEvent {
	return parseMoodleICS(raw)
}
...
// ParseICSEventLine parses a single ICS line and updates the event. Exported helper.
func ParseICSEventLine(line string, event *MoodleCalendarEvent) {
	parseICSEventLine(line, event)
}
...
// NormalizeLineEndings exported helper.
func NormalizeLineEndings(raw string) string { return normalizeLineEndings(raw) }
...
// SortEventsByDeadline exported helper.
func SortEventsByDeadline(events []MoodleCalendarEvent) { sortEventsByDeadline(events) }
...
// IcsValue exported helper.
func IcsValue(line string) string { return icsValue(line) }
...
// IcsPropertyValue exported helper.
func IcsPropertyValue(line string, property string) (string, bool) {
	return icsPropertyValue(line, property)
}
...
// UnescapeICS exported helper.
func UnescapeICS(value string) string { return unescapeICS(value) }
...
// CleanMoodleTitle exported helper.
func CleanMoodleTitle(title string) string { return cleanMoodleTitle(title) }
...
// ParseICalDate exported helper.
func ParseICalDate(raw string) (time.Time, bool) { return parseICalDate(raw) }
...
func ConvertGoogleDeadline(dueDate *GoogleDate, dueTime *GoogleTimeOfDay) (time.Time, bool) {
	return convertGoogleDeadline(dueDate, dueTime)
}
