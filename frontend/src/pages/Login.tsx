...
function LoginContent() {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Native Android: OS account picker -> serverAuthCode -> backend native endpoint.
    if (isNative()) {
      if (signingIn) return;
      setSigningIn(true);
      setError("");
      try {
        const serverAuthCode = await signInWithGoogleNative();
        const base = getNativeApiBase();
        const res = await fetch(`${base}/v1/auth/google/native`, {
          method: "POST",
        ...
        if (!res.ok || !data.access_token) {
          setError(mapErrorToMessage(data.error || "exchange_failed"));
          return;
        }
        ...
        if (msg === NATIVE_AUTH_NOT_CONFIGURED) {
          setError("Login native belum dikonfigurasi (VITE_GOOGLE_WEB_CLIENT_ID + Android OAuth client). Hubungi admin.");
        } else if (/cancel|cancelled|dismissed|12501/i.test(msg)) {
          // User dismissed the account picker — silent, no error banner.
        } else {
          setError(mapErrorToMessage("exchange_failed"));
        }
      } finally {
        setSigningIn(false);
      }
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl border border-black/5 p-6 sm:p-8">
            <button onClick={handleGoogleLogin} disabled={authStatus === "checking" || signingIn} className="w-full h-11 bg-white border border-black/10 rounded-full text-sm font-medium text-black hover:bg-black/[0.04] hover:border-black/15 transition-colors flex items-center justify-center gap-3 mb-6 disabled:opacity-60">
              {authStatus === "checking" ? (
                "Memeriksa sesi…"
              ) : signingIn ? (
