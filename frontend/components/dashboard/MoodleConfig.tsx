...
import { Link2, Sparkles, AlertCircle, Save, Loader2, CheckCircle2 } from 'lucide-react'
...
export function MoodleConfig({ url, onSave, onTest, isLoading, isTesting }: MoodleConfigProps) {
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <label className="text-sm font-black uppercase tracking-widest text-slate-400 pl-1">URL Ekspor Moodle</label>
        ...
        <div className="text-[11px] text-slate-400 pl-1 flex flex-col gap-2 leading-relaxed">
          <span className="flex items-center gap-1"><AlertCircle className="size-3 shrink-0" /> Dapatkan URL ini di Moodle ITERA {'>'} Calendar {'>'} Export Calendar {'>'} Get calendar URL.</span>
          <div className="flex items-center gap-2 ml-4">
            <img src="https://upload.wikimedia.org/wikipedia/commons/e/e0/Logo_ITERA.png" alt="ITERA Logo" className="h-6 object-contain" />
            <span className="font-bold text-blue-500/80">Compatible with kuliah2.itera.ac.id</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-[2rem] p-8 space-y-6">
        <h4 className="text-sm font-black text-blue-900 uppercase tracking-widest flex items-center gap-2">
            <Sparkles className="size-4" /> Cara Mendapatkan URL Moodle
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
                {[
                    ...
                    <div key={i} className="flex items-start gap-3">
                        <div className="size-5 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-black shrink-0 mt-0.5">{i+1}</div>
            ...
            <div className="bg-white rounded-2xl border border-blue-100 p-4 flex flex-col items-center justify-center text-center space-y-2">
                <div className="size-12 bg-blue-50 rounded-full flex items-center justify-center text-2xl">💡</div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tips</p>
                <p className="text-xs font-medium text-slate-600 leading-relaxed">Pastikan pilih <strong>&quot;Recent and next 60 days&quot;</strong> agar semua deadline terbaca oleh bot.</p>
            </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          {isTesting ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4 text-blue-600 group-hover:scale-110" />}
