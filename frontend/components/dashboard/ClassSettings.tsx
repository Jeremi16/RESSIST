        ...
        {availableCourses.length === 0 ? (
          ...
          <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
            {availableCourses.map((course) => {
              ...
              return (
                <div key={course.id} className="bg-white border border-black/5 rounded-2xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
      ...
      <AnimatePresence>
        {confirmMuteCourse && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 10 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-sm bg-white rounded-2xl p-5 z-50 shadow-xl">
              ...
              <p className="text-xs text-black/60 mt-1 leading-relaxed">Mata kuliah <span className="font-medium text-black">{confirmMuteCourse}</span> akan di-mute. Tugas dari mata kuliah ini tidak akan muncul di timeline dan notifikasi akan dimatikan. Kamu bisa mengaktifkannya kembali kapan saja.</p>
