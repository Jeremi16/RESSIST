    ...
    <div className="space-y-6 min-w-0 max-w-full overflow-x-clip">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <div className="size-8 bg-[#0059D0] rounded-xl flex items-center justify-center text-white shrink-0"><BookOpen className="size-4" /></div>
                    ...
                    <div className="min-w-0 flex-1">
                      {isEditing ? (
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <button onClick={() => saveAlias(course.name)} className="size-8 bg-[#0059D0] text-white rounded-full flex items-center justify-center shrink-0"><Check className="size-3.5" /></button>
                        ...
                        <div className="flex items-center gap-2 group min-w-0">
                          ...
                          <button onClick={() => startEditing(course.id, course.name)} aria-label="Ubah alias" className="opacity-100 lg:opacity-0 lg:group-hover:opacity-100 size-6 bg-black/5 rounded-full flex items-center justify-center shrink-0 hover:bg-[#0059D0] hover:text-white transition-colors"><Pencil className="size-3" /></button>
                        </div>
                      )}
                    </div>
                  </div>
                  <button type="button" onClick={() => handleMuteClick(course.name)} className={cn("h-7 px-3 rounded-full text-xs font-medium whitespace-nowrap shrink-0 transition-colors", muted.includes(course.name) ? "bg-black/10 text-black/40" : "bg-[#0059D0] text-white")}>{muted.includes(course.name) ? "Muted" : "Aktif"}</button>
                </div>
              );
            })}
          </div>
                ...
                <div key={course.id} className="bg-white border border-black/5 rounded-2xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0 max-w-full overflow-hidden">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap justify-start sm:justify-end shrink-0 min-w-0 max-w-full">
                    {allClassCodes.map((code) => (
                      <button key={code} type="button" onClick={() => togglePerCourse(course.name, code)} className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-colors", selected === code ? "bg-[#0059D0] border-[#0059D0] text-white" : "bg-white border-black/10 text-black/60 hover:border-black/20")}>{code}</button>
                    ))}
          ...
          <AnimatePresence>
            {showAddInput ? (
              <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} className="flex items-center gap-2">
                ...
                <button type="button" onClick={handleAddClassCode} disabled={!newClassCode.trim()} className="size-9 bg-[#0059D0] text-white rounded-full flex items-center justify-center disabled:opacity-50"><Plus className="size-4" /></button>
              </motion.div>
            ) : (
