  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        <h3 className="text-lg font-black text-slate-900">Konfigurasi Sumber Tugas</h3>
        <p className="text-sm text-slate-500">
          Aktifkan satu atau lebih sumber tugas. Sistem akan menggabungkan tugas dari semua sumber yang aktif.
        </p>
      </div>
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`size-14 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${
              moodleEnabledState ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-500'
            }`}>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {moodleEnabledState && <CheckCircle2 className="size-5 text-green-500" />}
              </div>
              <p className="text-sm text-slate-500">
                {moodleEnabledState ? 'Aktif' : 'Tidak aktif'} • Ambil dari kalender ICS Moodle
              </p>
            </div>
          </div>
      ...
      <div className="bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-sm">
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`size-14 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${
              googleEnabledState ? 'bg-green-500 text-white' : 'bg-green-50 text-green-500'
            }`}>
            ...
            <div>
              <div className="flex items-center gap-2">
                <h4 className="text-lg font-black text-slate-900">Google Classroom</h4>
                {googleEnabledState && googleConnected && <CheckCircle2 className="size-5 text-green-500" />}
                {googleEnabledState && !googleConnected && <AlertTriangle className="size-5 text-amber-500" />}
              </div>
              <p className="text-sm text-slate-500">
                {googleConnected 
                  ? (googleEnabledState ? 'Aktif • Terhubung otomatis' : 'Tidak aktif • Login ulang untuk aktifkan')
              ...
              <div className="p-6 space-y-4">
                <div className="bg-green-50 border border-green-100 rounded-2xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-bold text-green-900">Terhubung Otomatis</p>
                      <p className="text-sm text-green-700 mt-1">
                        Google Classroom terhubung secara otomatis saat login dengan akun Google ITERA. 
      ...
      {(moodleEnabledState || googleEnabledState) && (
        ...
        >
          ...
          <div>
            <h4 className="font-bold text-blue-900 mb-1">Penggabungan Tugas Otomatis</h4>
      ...
      <div className="flex flex-col gap-4 pt-4">
        ...
        >
          {isTesting ? (
            ...
            <Sparkles className="size-4 text-blue-600 group-hover:scale-110" />
          )}
