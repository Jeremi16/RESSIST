  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1.5">
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama Anda" className="w-full h-10 px-4 bg-[#F5F0EB] border border-black/5 rounded-full text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-black/10" />
      </div>

      <div className="space-y-1.5">
        <label className="text-xs font-medium text-black/40 flex items-center gap-1.5"><Phone className="size-3.5" /> WhatsApp</label>
        <input type="text" value={whatsappNumber} onChange={(e) => setWhatsappNumber(e.target.value)} placeholder="Nomor WhatsApp (ex: +62812...)" className="w-full h-10 px-4 bg-[#F5F0EB] border border-black/5 rounded-full text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-black/10" />
      ...
      <div className="space-y-1.5">
        ...
        <input type="text" value={telegramChatId} onChange={(e) => setTelegramChatId(e.target.value)} placeholder="Telegram Chat ID Anda" className="w-full h-10 px-4 bg-[#F5F0EB] border border-black/5 rounded-full text-sm text-black placeholder:text-black/30 focus:outline-none focus:border-black/10" />
        <div className="flex flex-wrap items-center gap-2 text-xs text-black/60">
          ...
          {userData.telegram_chat_id && <button type="button" onClick={() => copyToClipboard(userData.telegram_chat_id!, "Chat ID Telegram disalin!")} className="inline-flex items-center gap-1 px-2.5 py-1 bg-black text-white rounded-full text-xs hover:bg-black/90 transition-colors"><Copy className="size-3" /> Salin Chat ID</button>}
        </div>
      </div>
      ...
      <div className="pt-4 border-t border-black/5 flex flex-col gap-3">
        ...
        {saveSuccess && <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="p-3 bg-black text-white rounded-2xl flex items-center gap-2 text-sm"><CheckCircle2 className="size-4" /> Profil berhasil disimpan.</motion.div>}
      </div>
    </div>
  );
}
