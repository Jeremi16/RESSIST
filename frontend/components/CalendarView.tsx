...
interface Event { title: string; course: string | null; class_code?: string | null; deadline: Date | string; full_title?: string; original_course?: string | null; completed?: boolean; status?: string; }
...
export function CalendarView({ events }: CalendarViewProps) {
  ...
  // Filter out completed/missed tasks — calendar only shows pending
  const visibleEvents = useMemo(() => events.filter((e) => (e.status ? e.status === "pending" : !e.completed)), [events]);
            ...
            <div key={day.toISOString()} className={cn("min-h-[68px] p-1.5 border-r border-b border-black/5", !isSelectedMonth ? "opacity-30 bg-black/[0.01]" : "bg-white")}>
              ...
              <div className="mt-1 space-y-1">
                {dayEvents.slice(0, 2).map((event, idx) => (
                  <div key={idx} className="px-1.5 py-0.5 rounded-full text-[10px] font-medium truncate bg-[#0059D0] text-white" title={event.full_title || event.title}>
                    {event.title}
                  </div>
                ))}
